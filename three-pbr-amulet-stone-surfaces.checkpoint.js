...
const Q5_CERAMIC_MATERIAL = {
  hope: { preset: 'dragon', attenuationColor: [0.99, 0.99, 1.0] },
  excitement: { preset: 'chrome', color: 0xffffff },
  fear: { preset: 'chrome', color: 0x525860 },
  confusion: { preset: 'dragon', attenuationColor: [0.99, 0.99, 1.0] },
  impatience: {
    preset: 'matte',
    color: 0x929490,
    roughness: 0.98,
...
function sampleSoftStonewareSignsNoise(u, v) {
  return sampleSoftStonewareNoise(u, v, 19.4);
}
...
function paintStoneProcTexels(variantKey, s, edgeWear) {
  ...
  if (variantKey === 'soft_stoneware_signs') {
    return paintSoftStonewareTexels(SOFT_STONEWARE_PAINT[variantKey], s, edgeWear);
  }
...
/** @deprecated — use organicSlabFrameStyle2 */
...
const SLAB_FRAME_TUBE_MAX_PTS = 680;
...
const Q3_THREAD_INLAY_SINK = 0.12;
/** @deprecated — Q3 no longer behind stone */
const Q3_THREAD_BEHIND_STONE_Z_OFFSET = 0.06;
/** Q1 ceramic — unified tubes on stone front (original thickness). */
...
/** Ceramic sits in stone bed — slightly below shoulder rim (inlay, not floating). */
const Q1_CERAMIC_INLAY_FLUSH = 0.58;
const SLAB_CERAMIC_INLAY_BED_DEPTH = 0.68;
const SLAB_CERAMIC_INLAY_SHOULDER = 0.54;
const SLAB_CERAMIC_INLAY_SEAT = 0.74;
const SLAB_CERAMIC_INLAY_COLLAR = 0.98;
...
function buildCeramicQ1StoneWrapIntegration(rootSvg, slabMask, style3, style2, stoneTubeR) {
  ...
  if (metalHaloWrap) {
    metalHaloWrap.wrapBand = roundR * 2.95;
    metalHaloWrap.collarH = stoneTubeR * SLAB_CERAMIC_INLAY_COLLAR;
    metalHaloWrap.seatDepth = roundR * SLAB_CERAMIC_INLAY_SEAT;
  }
  return {
    ceramicMask,
    metalHaloWrap,
    ...
    metalBedTubeR: tubeR,
    metalBedDepth: roundR * SLAB_CERAMIC_INLAY_BED_DEPTH,
    metalBedShoulder: roundR * SLAB_CERAMIC_INLAY_SHOULDER,
...
function ceramicQ1TubeZ(stoneMesh, style3, style2, stoneTubeR) {
  stoneMesh.geometry.computeBoundingBox();
  const stoneTop = stoneMesh.geometry.boundingBox.max.z * stoneMesh.scale.z;
  const roundR = stoneTubeR * 1.25;
  const bedDepth = roundR * SLAB_CERAMIC_INLAY_BED_DEPTH;
  const tubeRadius =
    frameTubeBaseRadius(style3) *
    SLAB_Q1_CERAMIC_TUBE_RADIUS_SCALE *
    frameRadiusScaleFromStyle2(style2);
  const inlaySink = bedDepth * Q1_CERAMIC_INLAY_FLUSH;
  return stoneMesh.position.z + stoneTop - inlaySink + tubeRadius * 0.2;
}
...
function addCeramicQ1CenterLayer(
  ...
  let polylines = collectCeramicQ1TubePolylines(rootSvg, stoneSlabMask, style3);
  if (!polylines.length) return { count: 0 };

  const radiusScale = SLAB_Q1_CERAMIC_TUBE_RADIUS_SCALE * frameRadiusScaleFromStyle2(style2);
  const tubeRadius = frameTubeBaseRadius(style3) * radiusScale;
  polylines = roundCeramicTubePolylines(polylines, tubeRadius);

  const z = ceramicQ1TubeZ(stoneMesh, style3, style2, l3TubeRadius(style3) * STONE_L3_THICKNESS);
...
function q3MetalThreadZ(stoneMesh, style3, style2, stoneTubeR) {
  stoneMesh.geometry.computeBoundingBox();
  const stoneTop = stoneMesh.geometry.boundingBox.max.z * stoneMesh.scale.z;
  const stoneZ = stoneMesh.position.z + stoneTop;
  const ceramicZ = ceramicQ1TubeZ(stoneMesh, style3, style2, stoneTubeR);
  if (ceramicZ > stoneZ + 0.02) {
    return stoneZ + (ceramicZ - stoneZ) * 0.45;
  }
  return stoneZ + 0.1;
}
...
function addQ3MetalThreadLayer(
  ...
  if (!polylines.length) return 0;
  const stoneTubeR = l3TubeRadius(style3) * STONE_L3_THICKNESS;
  const z = q3MetalThreadZ(stoneMesh, style3, style2, stoneTubeR);
...
function buildExactGapFrameContourFromGrid(grid, w, h, maskOrigin, style3, style2, xyScale = 1) {
  ...
  const rough = stoneRoughnessFromStyle(style2);
  const step = SLAB_FRAME_CONTOUR_SUBSAMPLE * (1 - rough * 0.38);
  return subsampleContourPtsPreserveCorners(pts, Math.max(PATH_STEP * 0.08, step));
}
