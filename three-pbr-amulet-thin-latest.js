/**
 * SVG stroke paths → Three.js PBR (metal tubes L2, thin ceramic tubes L3).
...
function addLights(scene, ageFactor = 0) {
  const key = 1 - ageFactor * 0.32;
  const cool = 1 - ageFactor * 0.28;
  const warm = 1 - ageFactor * 0.22;
...
/** גיל — הכהיית גוון תחום (שכבה 3) */
function ageDarkenColor(hexColor, ageFactor) {
  const base = new THREE.Color(hexColor || '#D7A2B4');
  const deep = new THREE.Color(0x2a1f18);
  base.lerp(deep, ageFactor * 0.4);
  base.multiplyScalar(Math.max(0.52, 1 - ageFactor * 0.3));
  return base;
}
...
function buildCeramicMaterial(hexColor, style3, ageNum) {
  ...
  const clearcoat = Math.max(0.45, 1.0 - ageFactor * 0.48);
  const clearcoatRoughness = 0.01 + ageFactor * 0.32;
  const thickness = 4 + ageFactor * 8;
  const transmission = 0.58 - ageFactor * 0.22;

  const mat = new THREE.MeshPhysicalMaterial({
    color: ageDarkenColor(hexColor, ageFactor),
    metalness: 0.0,
    roughness,
    transmission,
    thickness,
    ...
    clearcoat,
    clearcoatRoughness,
    envMapIntensity: Math.max(1.1, 2.8 - ageFactor * 1.6),
    transparent: true,
...
function buildStrokeCurve(pts, style3, straight) {
  ...
  const pathAmp = gender === 'female' ? 2.5 : gender === 'male' ? 1.2 : 1.8;
  for (let i = 1; i < s.length - 1; i++) {
    const x = s[i].x;
    const y = s[i].y;
...
function applyOrganicDisplacement(geom, gender, tubeRadius, ageNum, occupationKey, metalLayer, surfaceScale) {
...
function addTubeFromPoints(
  ...
  const curve = buildStrokeCurve(pts, style3, isPolishedMetal);
  ...
  const radialSegs = isPolishedMetal ? 40 : gender === 'nonbinary' ? 4 : gender === 'male' ? 10 : 26;
...
function tryFrameTube(pts, material, scene, z, renderOrder, style3, ageNum) {
  if (pts.length < 2) return false;
  const reduced = downsamplePoints(pts, 160);
  return addTubeFromPoints(reduced, material, scene, z, renderOrder, style3, ageNum, true, 1.15, null);
}

/** מסגרת מ-SVG — מעגל (הגנה) או נתיב מותאם (זימון), עם fallback מ-bbox של L2+L3 */
function addFrameRing(mount, material, scene, z, renderOrder, style3, ageNum) {
  ...
  if (pathEl) {
    ...
    if (tryFrameTube(pts, material, scene, z, renderOrder, style3, ageNum)) return 1;
  }
  ...
  return tryFrameTube(pts, material, scene, z, renderOrder, style3, ageNum) ? 1 : 0;
}
...
async function renderPbrCore(svg, opts) {
  ...
  renderer.toneMappingExposure = Math.max(0.95, 1.5 * (1 - l3AgeFactorVal * 0.3));
  renderer.outputColorSpace = THREE.SRGBColorSpace;
  active.renderer = renderer;

  const scene = new THREE.Scene();
  active.envMap = setupEnvironment(renderer, scene);
  addLights(scene, l3AgeFactorVal);
  ...
  const tubesL3 = addTubesFromLayer(layer3, mount, ceramicMat, scene, 2, 1, style3, opts.ageNum);
  ...
  const tubesFrame = addFrameRing(mount, frameMat, scene, 10, 12, style3, opts.ageNum);
