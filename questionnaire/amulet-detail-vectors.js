...
const DETAIL_VECTOR_BOX_W = 224.395;
const DETAIL_VECTOR_BOX_H = 216.789;
const DETAIL_VECTOR_PAD = 16;
const DETAIL_VECTOR_SCALE = 0.88;
...
async function resolveDetailPageCompose(record, idx) {
  ...
  if (entryId != null) {
    ...
    if (perEntry?.svg) return perEntry;
  }
...
export async function renderResultOverlayVectors(answers) {
  ...
  const vectorMap = {
    ...
    resultVectorTiming: '.layer-q3-stone-engrave',
  };
  ...
  try {
    ...
    await Promise.all(
      Object.keys(vectorMap).map(function (containerId) {
        return extractAndRender(
          containerId,
          svgEl,
          vectorMap[containerId],
          viewBox,
          defsEl,
          composed.style2,
          composed.style3
...
function bboxInRootCoords(el, svgRoot) {
  const bb = el.getBBox();
  const ctm = el.getCTM();
  if (!ctm) return bb;
...
function smoothContour(points) {
  ...
  return chaikinSmoothClosed(subsampled, CONTOUR_CHAIKIN_PASSES);
}
...
async function unionLayerContour(layers, defsEl, svgEl, selector, style2, style3) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      ...
      const bh = Math.max(1, Math.round(crop.height * scale));
      let grid = gridFromImageData(data, bw, bh);
      if (countGridFilled(grid) < 16) continue;

      const raw = contourFromRaster(data, bw, bh, Math.round(config.dilate * scale));
      if (!raw) continue;
...
function detailVectorFitLayout(bounds) {
...
function mountLayerForPathExtract(svgEl, selector) {
  const mount = mountComposedSvg(svgEl);
  hideOtherLayers(mount, selector);
  mount.querySelectorAll('[filter]').forEach(function (el) {
    el.removeAttribute('filter');
  });
  return mount;
}

function collectLayerPathData(mountedSvg, selector) {
  const paths = [];
  mountedSvg.querySelectorAll(selector + ' path, ' + selector + ' .path-main').forEach(function (el) {
    const d = el.getAttribute('d');
    if (!d || d.length < 4) return;
    try {
      const bb = bboxInRootCoords(el, mountedSvg);
      if (bb.width > 0 && bb.height > 0) {
        paths.push({ d: d, bb: bb });
      }
    } catch (_) {
      paths.push({ d: d, bb: null });
    }
  });
  return paths;
}

function renderContourSvg(contour, container) {
  const pathD = closedCatmullRomPath(contour);
  ...
  const half = DETAIL_VECTOR_STROKE / 2;
...
function renderDirectLayerPaths(svgEl, selector, container) {
  const mountedSvg = mountLayerForPathExtract(svgEl, selector);
  try {
    const pathData = collectLayerPathData(mountedSvg, selector);
    ...
    pathData.forEach(function (item) {
      if (item.bb) {
        ...
        maxX = Math.max(maxX, item.bb.x + item.bb.width);
        maxY = Math.max(maxY, item.bb.y + item.bb.height);
      }
    });
...
async function extractAndRender(containerId, svgEl, selector, viewBox, defsEl, style2, style3) {
  ...
  if (!contour || contour.length < 8) {
    console.warn('[detail-vectors] union contour empty for', selector, '— using direct paths');
    renderDirectLayerPaths(svgEl, selector, container);
    return;
  }

  if (!renderContourSvg(contour, container)) {
    renderDirectLayerPaths(svgEl, selector, container);
  }
}
