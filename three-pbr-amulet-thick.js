...
function buildCeramicMaterial(hexColor, style3, ageNum) {
  ...
  const roughness = 0.008 + ageFactor * 0.14 + ageFactor * ageFactor * 0.08;
  const clearcoat = Math.max(0.12, 1.0 - ageFactor * 0.72);
  const clearcoatRoughness = 0.012 + ageFactor * 0.38;
  const thickness = 3 + ageFactor * 7;
  const transmission = 0.52 - ageFactor * 0.28;
  ...
  if (ageBump) {
    mat.bumpMap = ageBump;
    const texBoost = l3TextureBoost(style3);
    mat.bumpScale = (0.22 + ageFactor * 0.72) * texBoost;
  }
  return mat;
}
...
function applyOrganicDisplacement(
  ...
  if (metalLayer) {
    ...
    const rough = 1 - smooth;
    if (isFrame) return;
    amp = tubeRadius * (rough * rough * 0.85 + rough * 0.32);
    freq = 0.014 + rough * 0.08;
  } else {
    const ageFactor =
...
function addTubeFromPoints(
  ...
  const smoothNameLayer = !!style2 && !isFrame;
  const curve = buildStrokeCurve(
    pts,
    style3,
    isPolishedMetal || smoothNameLayer,
    smoothNameLayer ? null : style2?.occupationKey,
    isFrame
  );
...
/** מרחיב את מסכת L3 — מונע קצה שטוח/חתוך בגבול המשטח המנופח */
...
function collectLayerPolylines(layerEl, rootSvg, strokeScene) {
  const domePad = strokeScene * 0.55 + 8;
  const polylines = [];
  const add = (pts, closed) => {
    if (pts.length < 2) return;
    const drawPts = closed ? pts : extendOpenPathCaps(pts, domePad);
...
function collectLayer3Polylines(layerEl, rootSvg, style3) {
  ...
  const strokeScene = L3_STROKE_WIDTH + domeR + 8;
  return collectLayerPolylines(layerEl, rootSvg, strokeScene);
}

function l2StrokeScene(style2) {
  const gender = style2?.gender || 'female';
  const base = gender === 'nonbinary' ? L3_STROKE_WIDTH * 1.08 : gender === 'male' ? L3_STROKE_WIDTH * 1.02 : L3_STROKE_WIDTH * 1.05;
  return base + TUBE_RADIUS * 0.6;
}

function collectLayer2Polylines(layerEl, rootSvg, style2) {
  return collectLayerPolylines(layerEl, rootSvg, l2StrokeScene(style2));
}
...
/** Raster union mask — קנבס לפי bbox אמיתי של שכבה */
function rasterizePolylinesMask(polylines, strokeScene, label) {
  const maskOrigin = maskBoundsFromPolylines(polylines, strokeScene);
  if (!maskOrigin) throw new Error(label + ' mask bounds empty');
...
function rasterizeLayer3MaskCanvas(layerEl, rootSvg, style3) {
  const { polylines, strokeScene } = collectLayer3Polylines(layerEl, rootSvg, style3);
  return rasterizePolylinesMask(polylines, strokeScene, 'L3');
}

function rasterizeLayer2MaskCanvas(layerEl, rootSvg, style2) {
  const { polylines, strokeScene } = collectLayer2Polylines(layerEl, rootSvg, style2);
  return rasterizePolylinesMask(polylines, strokeScene, 'L2');
}
 ...
 */
function buildInflatedMeshFromMask(grid, w, h, radius, ageNum, maskOrigin, style3) {
  const dist = distanceTransform(grid, w, h);
  const strength = l3BumpStrength(style3);
  const step = strength >= 0.55 ? 1 : MASK_MESH_STEP;
  ...
  applyMassivePillDisplacement(geom, style3, ageNum, radius);
  geom.computeVertexNormals();
  return geom;
}

function buildUnifiedLayer3Geometry(layerEl, rootSvg, style3, ageNum) {
  const { canvas, maskOrigin } = rasterizeLayer3MaskCanvas(layerEl, rootSvg, style3);
  ...
  return buildInflatedMeshFromMask(dilated, w, h, radius, ageNum, maskOrigin, style3);
}
