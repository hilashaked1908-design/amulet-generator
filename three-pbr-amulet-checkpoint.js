...
/** z משותף ל-L2 ו-L3 — הזכוכית צמודה למתכת; renderOrder שומר L2 מעל */
...
function applyL3VertexColors(geom, maskOrigin, tubeRadius, distToL2 = null, maskW = 0, maskH = 0) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const screen = 1 - (1 - tmp.r) * (1 - specular * 0.62);
    tmp.r = screen;
    const screenG = 1 - (1 - tmp.g) * (1 - specular * 0.62);
    tmp.g = screenG;
    const screenB = 1 - (1 - tmp.b) * (1 - specular * 0.58);
    tmp.b = screenB;
...
const L3_MORPH_DILATE_PX = 22 * MASK_SCALE;
...
function dilateMaskGrid1px(grid, w, h) {
  return dilateMaskGrid(grid, w, h, 1);
}
...
function distanceTransformPerComponent(grid, labels, componentCount, w, h) {
  ...
  for (let label = 1; label <= componentCount; label++) {
    const compGrid = new Uint8Array(grid.length);
    for (let i = 0; i < grid.length; i++) compGrid[i] = labels[i] === label ? 1 : 0;
    const compDist = distanceTransform(compGrid, w, h);
    for (let i = 0; i < grid.length; i++) {
      if (labels[i] === label) dist[i] = compDist[i];
    }
  }
  return dist;
}
...
function buildUnifiedLayer3Geometry(layerEl, rootSvg, style3, ageNum, layer2El = null, style2 = null) {
  ...
  const l3Dilated = dilateMaskGrid(l3Grid, w, h, L3_MORPH_DILATE_PX * 2);

  let filled = 0;
  for (let i = 0; i < l3Dilated.length; i++) filled += l3Dilated[i];
  if (filled < 80) throw new Error('L3 union mask empty (' + filled + ' px)');

  let meshGrid = l3Dilated;
  let distOverride = null;
  let distToL2 = null;

  if (layer2El && style2) {
    const l2ShadowGrid = rasterizeLayer2MaskGrid(layer2El, rootSvg, style2, style3, maskOrigin, w, h);
    distToL2 = distanceToMaskGrid(l2ShadowGrid, w, h);

    const l2Grid = rasterizeLayer2GapMaskGrid(layer2El, rootSvg, style2, style3, maskOrigin, w, h);
    const gapGrid = subtractMaskGrid(l3Dilated, l2Grid);
    ...
    if (gapFilled >= 20) {
      ...
      if (componentCount > 0) {
        distOverride = distanceTransformPerComponent(gapGrid, labels, componentCount, w, h);
        meshGrid = gapGrid;
      }
    }
  }
...
async function renderPbrCore(svg, opts) {
  ...
  const tubesL3 = addUnifiedSolidFromLayer(layer3, mount, ceramicMat, scene, L2_SURFACE_Z, 10, style3, opts.ageNum, layer2, style2);
