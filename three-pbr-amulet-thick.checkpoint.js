...
function getStudioEnvMap(renderer) {
  if (sharedStudioEnvMap) return sharedStudioEnvMap;
...
function frameSmoothnessFromStyle2(style2) {
  ...
  return style2?.occupationSmoothness ?? 0.5;
}
...
/** Q5 frame bump presets — smoothness / spikiness by feeling answer */
const Q5_FRAME_BUMP = {
  hope: { bumpMul: 0.08, bumpLin: 0, freq: 0.22, octaves: 1 },
  longing: { bumpMul: 0.22, bumpLin: 0.06, freq: 0.18, octaves: 2 },
  excitement: { bumpMul: 0.32, bumpLin: 0.1, freq: 0.14, octaves: 2 },
  fear: { bumpMul: 0.42, bumpLin: 0.14, freq: 0.11, octaves: 3 },
  impatience: { bumpMul: 0.52, bumpLin: 0.18, freq: 0.08, octaves: 4 },
  confusion: { bumpMul: 0.62, bumpLin: 0.24, freq: 0.05, octaves: 5 },
};

const frameBumpTextureCache = {};

function createFrameBumpTexture(style2) {
  const rough = frameRoughnessFromStyle2(style2);
  if (rough < 0.04) return null;
  const feeling = style2?.q5Feeling || 'hope';
  const preset = Q5_FRAME_BUMP[feeling] ?? Q5_FRAME_BUMP.hope;
  const cacheKey = feeling + ':' + rough.toFixed(2);
  if (frameBumpTextureCache[cacheKey]) return frameBumpTextureCache[cacheKey];
  ...
  const turb = feeling === 'impatience' || feeling === 'confusion';
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const i = (y * size + x) * 4;
      let n =
        Math.sin(x * freq + y * freq * 0.61) * 0.35 +
        Math.sin(x * freq * 2.1 - y * freq * 1.3) * 0.25;
      if (turb) {
        n +=
          Math.sin(x * freq * 0.41 + y * freq * 0.87 + 1.7) * 0.22 +
          Math.cos(x * freq * 1.6 - y * freq * 0.52) * 0.18;
      }
      n += (Math.random() - 0.5) * (turb ? 0.35 : 0.2);
      const v = 128 + n * rough * (turb ? 130 : 100);
      img.data[i] = img.data[i + 1] = img.data[i + 2] = Math.max(80, Math.min(200, v));
      img.data[i + 3] = 255;
    }
  }
  ...
  frameBumpTextureCache[cacheKey] = tex;
  return tex;
}

function buildFrameMetalMaterial(style2, envMap = null) {
  const rough = frameRoughnessFromStyle2(style2);
  const feeling = style2?.q5Feeling || 'hope';
  const preset = Q5_FRAME_BUMP[feeling] ?? Q5_FRAME_BUMP.hope;
  const mat = new THREE.MeshStandardMaterial({
    color: 0x888888,
    metalness: 1.0,
    roughness: Math.min(0.38, 0.02 + rough * rough * 0.3 + rough * 0.1),
    envMapIntensity: envMap ? 0.75 + (1 - rough) * 0.75 : 1.5,
  });
  const bumpTex = createFrameBumpTexture(style2);
  if (bumpTex) {
    mat.bumpMap = bumpTex;
    mat.bumpScale = rough * rough * preset.bumpMul + rough * preset.bumpLin;
  }
...
function buildCeramicMaterial(hexColor, style3, ageNum, l3Spike) {
  ...
  const transmission = 0.42 - ageFactor * 0.08;
  const roughness = 0.48;
...
function buildProceduralStoneTextures(warm = false) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      const bumpV = Math.round(
        Math.max(0, Math.min(255, (0.34 + s.height * 0.58) * 255))
      );
      const roughV = Math.round(
        Math.max(210, Math.min(255, 232 + s.sand * 8 + s.combined * 12))
      );
  ...
  normalCtx.putImageData(
    new ImageData(buildNormalMapFromHeights(heights, size, warm ? 10.5 : 11.5), size, size),
    0,
    0
  );
...
const SLAB_Q1_CERAMIC_PLACEMENT = { cxFrac: 0.5, cyFrac: 0.5, widthFrac: 0.56, heightFrac: 0.52, fit: 0.9 };
...
const SLAB_Q3_METAL_PLACEMENT = { cxFrac: 0.5, cyFrac: 0.5, widthFrac: 0.9, heightFrac: 0.74, fit: 1.14 };
...
/** Q3 metal threads — above stone (12), below ceramic (40). */
const Q3_THREAD_RENDER_ORDER = 38;
/** Q3 metal threads — same tube radius scale as slab frame. */
const Q3_THREAD_RADIUS_SCALE = SLAB_STONE_FRAME_RADIUS_SCALE;
...
function applyOrganicDisplacement(
  ...
  if (occupationKey === 'tech_finance' && !isFrame) return;
  ...
  if (isFrame) {
    ...
    const ampScale = 0.55 + Math.min(1, frameAmpMul) * 0.75;
...
function addTubeFromPoints(
  ...
  const frameAmpMul = isFrame ? style2?.frameContour?.wobbleMix ?? 1 : 1;
  const smooth = isFrame
...
function addStoneSlabMetalFrame(slabMask, material, scene, z, renderOrder, style3, ageNum, style2, stoneMesh = null) {
  const pts = stoneMesh
...
/**
 * Q1 connected glyphs → unified ceramic volume (saved-roughness L3 material), centered on stone slab.
 */
function buildCeramicQ1CenterGeometry(rootSvg, stoneSlabMask, style3, ageNum) {
  const el = rootSvg.querySelector('.layer-q1-ceramic') || rootSvg.querySelector('.layer-3');
  if (!el || !stoneSlabMask?.maskOrigin) return null;

  let pack = collectLayer3Polylines(el, rootSvg, style3);
  if (!pack.polylines?.length) {
    pack = collectGlyphLayerPolylines(el, rootSvg, style3);
  }
  if (!pack.polylines?.length) return null;

  const placement = SLAB_Q1_CERAMIC_PLACEMENT;
  const box = sceneTextBox(stoneSlabMask.maskOrigin, placement);
  const polylines = transformPolylinesToBox(pack.polylines, box, placement.fit);
  const strokeScene = pack.strokeScene || l3TubeRadius(style3) * 2;
  const maskOrigin = maskBoundsFromPolylines(polylines, strokeScene * 0.55);
  if (!maskOrigin) return null;

  let { grid, w, h } = rasterizePolylinesToGrid(polylines, strokeScene, maskOrigin);
  grid = dilateMaskGrid(grid, w, h, L3_MORPH_DILATE_PX);
  ...
  const gender = style3?.gender || 'female';
  applyOrganicDisplacement(geom, gender, l3TubeRadius(style3), ageNum, null, false, style3?.surfaceScale ?? 0);
  geom.computeVertexNormals();
  return geom;
}

function addCeramicQ1CenterLayer(scene, stoneMesh, rootSvg, stoneSlabMask, style3, ageNum, domainHex, l3Spike) {
  if (!stoneMesh || !stoneSlabMask) return 0;
  let geom;
  try {
    geom = buildCeramicQ1CenterGeometry(rootSvg, stoneSlabMask, style3, ageNum);
  } catch (err) {
    console.warn('[pbr] Q1 ceramic center failed:', err);
    return 0;
  }
  if (!geom) return 0;

  const ceramicMat = buildCeramicMaterial(domainHex, style3, ageNum, l3Spike);
  const mesh = new THREE.Mesh(geom, ceramicMat);
  ...
  mesh.position.z = stoneMesh.position.z + stoneTop + 0.05;
  mesh.renderOrder = 40;
  scene.add(mesh);
  return 1;
}

function collectQ3LayerPolylines(rootSvg, slabMask, style3, questionnaire, placement) {
...
/** Radial bridge segments from Q3 path ends to the metal frame contour. */
function appendQ3FrameBridgePolylines(polylines, slabMask, style3) {
  ...
  framePts = framePts.map(
    (p) => new THREE.Vector3(p.x / scale, p.y / scale, 0)
  );
...
function q3MetalThreadZ(stoneMesh) {
  ...
  const zLocal = bb.min.z + (bb.max.z - bb.min.z) * 0.82;
  return stoneMesh.position.z + zLocal * stoneMesh.scale.z + 0.018;
}

/** Q3 answer glyphs as frame-metal tubes on the stone surface (below ceramic). */
function addQ3MetalThreadLayer(scene, rootSvg, stoneMesh, slabMask, style2, style3, ageNum, questionnaire = null) {
  if (!stoneMesh || !slabMask?.maskOrigin) return 0;
  const polylines = collectQ3MetalThreadPolylines(rootSvg, slabMask, style3, questionnaire);
  if (!polylines.length) return 0;
  const envMap = active?.envMap ?? getStudioEnvMap(active.renderer);
  const mat = buildFrameMetalMaterial(style2, envMap);
  const z = q3MetalThreadZ(stoneMesh);
  return addTubesFromPolylines(polylines, mat, scene, z, Q3_THREAD_RENDER_ORDER, style3, ageNum, style2, {
    xyScale: SLAB_STONE_XY_SCALE,
    radiusScale: Q3_THREAD_RADIUS_SCALE,
    isFrame: true,
  });
}
...
function buildStoneMeshFrameContour(stoneMesh, style3, style2) {
  ...
  r = smoothRadialRadii(r, Math.max(1, Math.round(1 + smooth * 3)));
  ...
  const margin = SLAB_STONE_FRAME_MARGIN + frameTubeBaseRadius(style3) * 1.15;
  const pts = [];
  for (let a = 0; a < 360; a++) {
  ...
  return subsampleContourPts(pts, PATH_STEP * 1.2);
}
...
async function buildUnifiedLayer3Geometry(
  ...
  if (l3MaterialMode === 'stone') {
    if (STONE_L3_SLAB_MODE) {
      const stoneShapeParams = questionnaire?.stoneShapeParams ?? null;
      let { grid: slabGrid, w, h, maskOrigin } = rasterizeSlabWrapMask(
        rootSvg,
        style2,
        style3,
        stoneShapeParams
...
async function renderPbrCore(svg, opts) {
  ...
  if (l3MaterialMode === 'stone' && STONE_L3_SLAB_MODE && stoneMesh && slabMask) {
    ...
    await yieldToMainThread();
    tubesQ1Ceramic = addCeramicQ1CenterLayer(
      scene,
      stoneMesh,
      mount,
      slabMask,
  ...
  const frameMat = style2
    ? buildFrameMetalMaterial(style2, active?.envMap ?? null)
    : new THREE.MeshStandardMaterial({
        color: 0x888888,
        metalness: 1.0,
        roughness: 0.15,
  ...
  if (l3MaterialMode === 'stone' && STONE_L3_SLAB_MODE && stoneMesh) {
